...
public class ImagedbUtil {
	...
	private final String TABLE_CREATED = "create table "
			+ TABLE_NAME
			+ " (_id integer primary key autoincrement,accounts text not null, "
			+ "pass text not null,created text not null);";
	private final String TABLE_NOT_EXISTS_CREATED = "create table if not exists "
			+ TABLE_NAME
			+ " (_id integer primary key autoincrement,image text not null, "
	...
	public Cursor getCursor(String... args) {
		Cursor mCursor = mdb.query(TABLE_NAME, args, null, null, null, null,
				null);
	...
	public Cursor getCursorArgs(String[] args, String[] selection) {
		Cursor mCursor = mdb.query(TABLE_NAME, args, ACCOUNTS + "=?",
	...
	public void addColumn() throws SQLException {
		String sql = "alter table " + TABLE_NAME + " add " + RECOG
				+ " text null;";
		mdb.execSQL(sql);
	}
